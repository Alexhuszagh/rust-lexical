...
if #[cfg(not(feature = "format"))] {
    ...
    #[inline]
    fn digit_separator_from_flags(flag: u64) -> u8 {
        (flag >> 56) as u8
    }
    ...
    const_assert!(flags0; NumberFormat::REQUIRED_INTEGER_DIGITS.bits == 1);
    check_subsequent_flags!(flags1; REQUIRED_INTEGER_DIGITS, REQUIRED_FRACTION_DIGITS);
    check_subsequent_flags!(flags2; REQUIRED_FRACTION_DIGITS, REQUIRED_EXPONENT_DIGITS);
    check_subsequent_flags!(flags3; REQUIRED_EXPONENT_DIGITS, NO_POSITIVE_MANTISSA_SIGN);
    check_subsequent_flags!(flags4; NO_POSITIVE_MANTISSA_SIGN, REQUIRED_MANTISSA_SIGN);
    check_subsequent_flags!(flags5; REQUIRED_MANTISSA_SIGN, NO_EXPONENT_NOTATION);
    check_subsequent_flags!(flags6; NO_EXPONENT_NOTATION, NO_POSITIVE_EXPONENT_SIGN);
    check_subsequent_flags!(flags7; NO_POSITIVE_EXPONENT_SIGN, REQUIRED_EXPONENT_SIGN);
    check_subsequent_flags!(flags8; REQUIRED_EXPONENT_SIGN, NO_EXPONENT_WITHOUT_FRACTION);
    check_subsequent_flags!(flags9; NO_EXPONENT_WITHOUT_FRACTION, NO_SPECIAL);
    check_subsequent_flags!(flags10; NO_SPECIAL, CASE_SENSITIVE_SPECIAL);
    check_subsequent_flags!(flags11; CASE_SENSITIVE_SPECIAL, NO_LEADING_ZEROS);

    // Digit separator flags.
    const_assert!(flags12; NumberFormat::INTEGER_INTERNAL_DIGIT_SEPARATOR.bits == 1 << 32);
    check_subsequent_flags!(flags13; INTEGER_INTERNAL_DIGIT_SEPARATOR, INTEGER_LEADING_DIGIT_SEPARATOR);
    check_subsequent_flags!(flags14; INTEGER_LEADING_DIGIT_SEPARATOR, INTEGER_TRAILING_DIGIT_SEPARATOR);
    check_subsequent_flags!(flags15; INTEGER_TRAILING_DIGIT_SEPARATOR, INTEGER_CONSECUTIVE_DIGIT_SEPARATOR);
    check_subsequent_flags!(flags16; INTEGER_CONSECUTIVE_DIGIT_SEPARATOR, FRACTION_INTERNAL_DIGIT_SEPARATOR);
    check_subsequent_flags!(flags17; FRACTION_INTERNAL_DIGIT_SEPARATOR, FRACTION_LEADING_DIGIT_SEPARATOR);
    check_subsequent_flags!(flags18; FRACTION_LEADING_DIGIT_SEPARATOR, FRACTION_TRAILING_DIGIT_SEPARATOR);
    check_subsequent_flags!(flags19; FRACTION_TRAILING_DIGIT_SEPARATOR, FRACTION_CONSECUTIVE_DIGIT_SEPARATOR);
    check_subsequent_flags!(flags20; FRACTION_CONSECUTIVE_DIGIT_SEPARATOR, EXPONENT_INTERNAL_DIGIT_SEPARATOR);
    check_subsequent_flags!(flags21; EXPONENT_INTERNAL_DIGIT_SEPARATOR, EXPONENT_LEADING_DIGIT_SEPARATOR);
    check_subsequent_flags!(flags22; EXPONENT_LEADING_DIGIT_SEPARATOR, EXPONENT_TRAILING_DIGIT_SEPARATOR);
    check_subsequent_flags!(flags23; EXPONENT_TRAILING_DIGIT_SEPARATOR, EXPONENT_CONSECUTIVE_DIGIT_SEPARATOR);
    check_subsequent_flags!(flags24; EXPONENT_CONSECUTIVE_DIGIT_SEPARATOR, SPECIAL_DIGIT_SEPARATOR);
