...
mod tests {
    ...
    fn special_bytes_test() {
        unsafe {
            ...
            assert!(atof32(b"INF").unwrap().is_infinite());
            assert!(atof32(b"Infinity").unwrap().is_infinite());
            assert_eq!(f64toa(f64::NAN, &mut buffer), b"NaN");
            assert_eq!(f64toa(f64::INFINITY, &mut buffer), b"inf");

            NAN_STRING.load_bytes(b"nan");
            INF_STRING.load_bytes(b"Infinity");

            assert!(atof32(b"inf").err().unwrap().code == ErrorCode::InvalidDigit);
            assert!(atof32(b"Infinity").unwrap().is_infinite());
            assert_eq!(f64toa(f64::NAN, &mut buffer), b"nan");
            assert_eq!(f64toa(f64::INFINITY, &mut buffer), b"Infinity");
    ...
    fn special_rounding_test() {
        ...
        unsafe {
            ...
            assert_eq!(atof64(b"-9007199254740993").unwrap(), -9007199254740992.0);
            assert_eq!(atof64(b"-9007199254740995").unwrap(), -9007199254740996.0);
            assert_eq!(atof64(b"9007199254740993").unwrap(), 9007199254740992.0);
            assert_eq!(atof64(b"9007199254740995").unwrap(), 9007199254740996.0);

            // Nearest, tie-away-zero
            FLOAT_ROUNDING = RoundingKind::NearestTieAwayZero;
            assert_eq!(atof64(b"-9007199254740993").unwrap(), -9007199254740994.0);
            assert_eq!(atof64(b"-9007199254740995").unwrap(), -9007199254740996.0);
            assert_eq!(atof64(b"9007199254740993").unwrap(), 9007199254740994.0);
            assert_eq!(atof64(b"9007199254740995").unwrap(), 9007199254740996.0);
            ...
            assert_eq!(atof64(b"-9007199254740993").unwrap(), -9007199254740992.0);
            assert_eq!(atof64(b"-9007199254740995").unwrap(), -9007199254740994.0);
            assert_eq!(atof64(b"9007199254740993").unwrap(), 9007199254740994.0);
            assert_eq!(atof64(b"9007199254740995").unwrap(), 9007199254740996.0);
    ...
    fn special_rounding_binary_test() {
        ...
        unsafe {
            // Nearest, tie-even
            FLOAT_ROUNDING = RoundingKind::NearestTieEven;
            assert_eq!(atof64_radix(2, b"-100000000000000000000000000000000000000000000000000001").unwrap(), -9007199254740992.0);
            assert_eq!(atof64_radix(2, b"-100000000000000000000000000000000000000000000000000011").unwrap(), -9007199254740996.0);
            assert_eq!(atof64_radix(2, b"100000000000000000000000000000000000000000000000000001").unwrap(), 9007199254740992.0);
            assert_eq!(atof64_radix(2, b"100000000000000000000000000000000000000000000000000011").unwrap(), 9007199254740996.0);

            // Nearest, tie-away-zero
            FLOAT_ROUNDING = RoundingKind::NearestTieAwayZero;
            assert_eq!(atof64_radix(2, b"-100000000000000000000000000000000000000000000000000001").unwrap(), -9007199254740994.0);
            assert_eq!(atof64_radix(2, b"-100000000000000000000000000000000000000000000000000011").unwrap(), -9007199254740996.0);
            assert_eq!(atof64_radix(2, b"100000000000000000000000000000000000000000000000000001").unwrap(), 9007199254740994.0);
            assert_eq!(atof64_radix(2, b"100000000000000000000000000000000000000000000000000011").unwrap(), 9007199254740996.0);
            ...
            FLOAT_ROUNDING = RoundingKind::TowardPositiveInfinity;
            assert_eq!(atof64_radix(2, b"-100000000000000000000000000000000000000000000000000001").unwrap(), -9007199254740992.0);
            assert_eq!(atof64_radix(2, b"-100000000000000000000000000000000000000000000000000011").unwrap(), -9007199254740994.0);
            assert_eq!(atof64_radix(2, b"100000000000000000000000000000000000000000000000000001").unwrap(), 9007199254740994.0);
            assert_eq!(atof64_radix(2, b"100000000000000000000000000000000000000000000000000011").unwrap(), 9007199254740996.0);
