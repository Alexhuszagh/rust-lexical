...
//! ## SyntaxFormat
//!
//! The syntax format defines **which** float constructs are valid
//! within, for example, if exponent notation is required or not
//! allowed. Since this dependent on the `feature = "format"`, it
//! also includes digit separators, since these are feature-gated
//! as well. See `options/syntax` for more details.
//!
//! ## LexerFormat
//!
//! The lexer format designs what characters are valid, that is, which
//! characters should be consider valid to continue tokenization.
//! For example, the exponent, decimal point control characters are
//! defined here. In addition, the radix, exponent base, and exponent
//! radix are defined here, since these features are mostly
//! dependent on the `feature = "radix"`, and do not incur
//! performance penalties when actived. See `options/lexer` for more details.
