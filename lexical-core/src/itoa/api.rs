...
#[inline]
fn unsigned<Narrow, Wide>(value: Narrow, radix: u32, buffer: &mut [u8]) -> usize
where
...
#[inline]
fn unsigned_with_options<Narrow, Wide>(
    value: Narrow,
    buffer: &mut [u8],
...
#[inline]
fn signed<Narrow, Wide, Unsigned>(value: Narrow, radix: u32, buffer: &mut [u8]) -> usize
where
...
#[inline]
fn signed_with_options<Narrow, Wide, Unsigned>(
    value: Narrow,
    buffer: &mut [u8],
    options: &WriteIntegerOptions,
) -> usize
...
#[cfg(all(test, feature = "write_integers"))]
mod tests {
    ...
    fn radix_test() {
        ...
        for (base, expected) in data.iter() {
            let options = WriteIntegerOptions::builder().radix(*base).build().unwrap();
    ...
    if #[cfg(feature = "parse_integers")] {
        ...
        fn u16_pow2_test() {
            ...
            let values: &[u16] = &[
                ...
                8193, 16383, 16384, 16385, 32767, 32768, 32769, 65535,
            ];
            for &i in values.iter() {
        ...
        fn u32_pow2_test() {
            ...
            let values: &[u32] = &[
                0, 1, 2, 3, 4, 5, 7, 8, 9, 15, 16, 17, 31, 32, 33, 63, 64, 65, 127, 128, 129, 255, 256,
                257, 511, 512, 513, 1023, 1024, 1025, 2047, 2048, 2049, 4095, 4096, 4097, 8191, 8192,
                8193, 16383, 16384, 16385, 32767, 32768, 32769, 65535, 65536, 65537, 131071, 131072,
                131073, 262143, 262144, 262145, 524287, 524288, 524289, 1048575, 1048576, 1048577,
                2097151, 2097152, 2097153, 4194303, 4194304, 4194305, 8388607, 8388608, 8388609,
                16777215, 16777216, 16777217, 33554431, 33554432, 33554433, 67108863, 67108864,
                67108865, 134217727, 134217728, 134217729, 268435455, 268435456, 268435457, 536870911,
                536870912, 536870913, 1073741823, 1073741824, 1073741825, 2147483647, 2147483648,
        ...
        fn u32_pow10_test() {
            ...
            let values: &[u32] = &[
                0, 1, 5, 9, 10, 11, 15, 99, 100, 101, 105, 999, 1000, 1001, 1005, 9999, 10000, 10001,
                10005, 99999, 100000, 100001, 100005, 999999, 1000000, 1000001, 1000005, 9999999,
                10000000, 10000001, 10000005, 99999999, 100000000, 100000001, 100000005, 999999999,
                1000000000, 1000000001, 1000000005,
            ];
            for &i in values.iter() {
                assert_eq!(i, u32::from_lexical(i.to_lexical(&mut buffer)).unwrap());
            }
