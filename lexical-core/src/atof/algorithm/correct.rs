    ...
    where F: FloatType
{
    ...
    let (mantissa, truncated) = match process_mantissa::<u64>(radix, &state) {
        Ok(v) => v,
        Err(c) => {
            let index = distance(bytes.as_ptr(), c);
            return Err((ErrorCode::InvalidDigit, offset + index).into());
        },
    };
    ...
    where F: FloatType
{
    ...
    let mut state = RawFloatState::new();
    state.parse(radix, bytes)?;
    let (mut mantissa, truncated) = match process_mantissa::<u64>(radix, &state) {
        Ok(v) => v,
        Err(c) => {
            let index = distance(bytes.as_ptr(), c);
            return Err((ErrorCode::InvalidDigit, offset + index).into());
...
mod tests {
    ...
    fn process_mantissa_test() {
        // 64-bits
        assert_eq!(Ok((12345, 0)), process_mantissa::<u64>(10, &new_state(b"1", b"2345", b"")));
        assert_eq!(Ok((12345, 0)), process_mantissa::<u64>(10, &new_state(b"12", b"345", b"")));
        assert_eq!(Ok((123456789, 0)), process_mantissa::<u64>(10, &new_state(b"12345", b"6789", b"")));
        assert_eq!(Ok((12345, 0)), process_mantissa::<u64>(10, &new_state(b"1", b"2345", b"10")));
        assert_eq!(Ok((10000000000000000000, 1)), process_mantissa::<u64>(10, &new_state(b"100000000000000000000", b"", b"")));
        assert_eq!(Ok((10000000000000000000, 1)), process_mantissa::<u64>(10, &new_state(b"100000000000000000001", b"", b"")));
        assert_eq!(Ok((17976931348623158079, 359)), process_mantissa::<u64>(10, &new_state(b"", b"9999999999999999999999999999999999999999999999999999999999999999999999", b"")));
        assert_eq!(Ok((1009, 0)), process_mantissa::<u64>(10, &new_state(b"1009", b"", b"-31")));

        // 128-bit
        assert_eq!(Ok((12345, 0)), process_mantissa::<u128>(10, &new_state(b"1", b"2345", b"")));
        assert_eq!(Ok((12345, 0)), process_mantissa::<u128>(10, &new_state(b"12", b"345", b"")));
        assert_eq!(Ok((123456789, 0)), process_mantissa::<u128>(10, &new_state(b"12345", b"6789", b"")));
        assert_eq!(Ok((12345, 0)), process_mantissa::<u128>(10, &new_state(b"1", b"2345", b"10")));
        assert_eq!(Ok((100000000000000000000, 0)), process_mantissa::<u128>(10, &new_state(b"100000000000000000000", b"", b"")));
        assert_eq!(Ok((100000000000000000001, 0)), process_mantissa::<u128>(10, &new_state(b"100000000000000000001", b"", b"")));
    }
    ...
    fn atof_test() {
        ...
        // Invalid
        assert_eq!(Err((ErrorCode::EmptyFraction, 0).into()), atof10(b"e10"));
