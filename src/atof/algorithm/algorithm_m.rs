...
{
    let small_powers = Bigint::small_powers(base);
...
mod tests {
    ...
    fn parse_mantissa_test() {
        assert_eq!(parse_mantissa("24703".bytes(), 10, usize::max_value()).data, from_u32(&[24703]));
        assert_eq!(parse_mantissa("24703282292062327208828439643411068618252990130716".bytes(), 10, usize::max_value()).data, from_u32(&[1661053468, 4066317011, 983098885, 3087178645, 3876945216, 16]));
        assert_eq!(parse_mantissa("2470328229206232720882843964341106861825299013071623822127928412503377536351043759326499181808179961".bytes(), 10, usize::max_value()).data, from_u32(&[343792377, 1257502172, 2486839483, 4245685716, 278612094, 1973156400, 2455112332, 951308983, 1039746225, 2266565483, 1156]));
