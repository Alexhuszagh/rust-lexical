...
impl Default for Bigint {
    fn default() -> Self {
        ...
        Bigint { data: Vec::with_capacity(20) }
    }
}
...
pub fn parse_mantissa<Iter>(mut digits: Iter, base: u32, max_digits: usize)
    -> Bigint
    where Iter: iter::Iterator<Item=u8>
{
    ...
    let mut counter = 0;
    let mut result = Bigint { data: Vec::with_capacity(count) };
    let mut value: u32 = 0;
    let mut i: usize = 0;
    loop {
        ...
        value *= base;
        value += char_to_digit(digit).as_u32();
...
fn max_digits_f32(base: u32) -> Option<usize> {
    match base {
        6  => Some(101),
        10 => Some(112),
        12 => Some(115),
...
fn max_digits_f64(base: u32) -> Option<usize> {
    match base {
        6  => Some(680),
        10 => Some(767),
        12 => Some(790),
...
fn max_digits<F>(base: u32)
    -> Option<usize>
    where F: Float
{
    match F::BITS {
        32 => max_digits_f32(base),
        64 => max_digits_f64(base),
        _  => unreachable!(),
    }
}
    ...
    -> i32
{
    ...
    if shift > 0 {
        ...
        // Need to shift the denominator left, and ensure it's divisible by 32.
        let shift = (-shift).as_usize();
        let s2 = nlz + (shift % 32);
        let s1 = shift + s2;
        num.ishl(s2);
...
pub unsafe fn atof<F, Iter>(digits: Iter, base: u32, sci_exponent: i32, f: F)
    -> F
          ...
          Iter: iter::Iterator<Item=u8>
{
    ...
    let max_digits = unwrap_or_max(max_digits::<F>(base));
    let count = max_digits.min(digits.size_hint().0);
    let exponent = sci_exponent + 1 - count.as_i32();

    if base.is_odd() && count > LARGE_POWER_MAX {
        // When we have extremely large values, it makes a lot more sense to
        // use am algorithm that scales linearly with input size. We
        // only precompute exponent up to 2^15 anyway for a given base, so
        // use it. If the base is not odd, we know the finite number of digits
        // for the worst-case representation, so we can create a valid ratio
        // and ignore the remaining digits.
        slow_atof(digits, base, sci_exponent, f)
    } else if exponent >= 0 {
        // Simple, we just need to multiply by the power of the base.
        ...
        let mut bigmant = parse_mantissa(digits, base, max_digits);
        bigmant.imul_power(base, exponent.as_u32());
        ...
        let mut num = parse_mantissa(digits, base, max_digits);
        let mut den = Bigint::from_u32(1);
        den.imul_power(base, exponent.as_u32());
...
mod tests {
    ...
    fn parse_mantissa_test() {
        assert_eq!(parse_mantissa("24703".bytes(), 10, usize::max_value()).data, [24703]);
        assert_eq!(parse_mantissa("24703282292062327208828439643411068618252990130716".bytes(), 10, usize::max_value()).data, [1661053468, 4066317011, 983098885, 3087178645, 3876945216, 16]);
