//! Measure the performance of the algom parser.
//! Data must be non-negative, non-special floating-point numbers.
//! These are slightly biased towards the algom results, since it
...
fn algom_f32_lexical(bench: &mut Bencher) {
    bench.iter(|| { F32_DATA.iter().for_each(|(x, e, f)| unsafe {
        black_box(algom_atof(x.bytes(), 10, *e, *f));
    })})
}
...
fn algom_f64_lexical(bench: &mut Bencher) {
...
benchmark_group!(f32_benches, algom_f32_lexical);
benchmark_group!(f64_benches, algom_f64_lexical);
