...
fn atof_malicious_f32_lexical(bench: &mut Bencher) {
    bench.iter(|| { F32_DATA.iter().for_each(|x| { black_box(atof32_slice(10, x.as_bytes())); } ) })
}

fn atof_malicious_f32_lexical_lossy(bench: &mut Bencher) {
    bench.iter(|| { F32_DATA.iter().for_each(|x| { black_box(atof32_lossy_slice(10, x.as_bytes())); } ) })
}
