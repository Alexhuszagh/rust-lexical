...
fn atof_malicious_f32_lexical(bench: &mut Bencher) {
    bench.iter(|| { F32_DATA.iter().for_each(|x| { black_box(atof32_slice(x.as_bytes())); } ) })
}

fn atof_malicious_f32_lexical_lossy(bench: &mut Bencher) {
    bench.iter(|| { F32_DATA.iter().for_each(|x| { black_box(atof32_lossy_slice(x.as_bytes())); } ) })
}
...
fn atof_malicious_f64_lexical(bench: &mut Bencher) {
    bench.iter(|| { F64_DATA.iter().for_each(|x| { black_box(atof64_slice(x.as_bytes())); } ) })
}

fn atof_malicious_f64_lexical_lossy(bench: &mut Bencher) {
    bench.iter(|| { F64_DATA.iter().for_each(|x| { black_box(atof64_lossy_slice(x.as_bytes())); } ) })
}
